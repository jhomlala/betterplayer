...
To automate creating a Pull Request, use the Dart script located at `scripts/create_pull_request.dart`. Make sure your PR body description is saved in a file (e.g., `pr_body.md`).
...
Run the following command from the root of the project:
```powershell
dart run scripts/create_pull_request.dart <TITLE> <HEAD_BRANCH> <BASE_BRANCH> <BODY_FILE_PATH> [owner/repo]
```
