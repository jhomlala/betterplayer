...
void main() {
  ...
  setUp(() {
    ...
    when(() => mockPlatform.setupLogCallback(any())).thenAnswer((_) async {});
    when(
      () => mockPlatform.create(
    ...
    when(
      () => mockPlatform.videoEventsFor(any()),
    ).thenAnswer((_) => eventController.stream);
    when(() => mockPlatform.setDataSource(any(), any())).thenAnswer((
      invocation,
    ) async {
    ...
    when(() => mockPlatform.setSpeed(any(), any())).thenAnswer((_) async {});
    ...
    when(
      () => mockPlatform.setTrackParameters(any(), any(), any(), any()),
    ).thenAnswer((_) async {});
  ...
  group('BetterPlayerController File Source tests', () {
    test('setupDataSource with file source uses file:// URI', () async {
      ...
      verify(
        () => mockPlatform.setDataSource(
          any(),
          any(
            that: predicate<DataSource>(
              (ds) =>
