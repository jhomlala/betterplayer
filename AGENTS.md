...
- **Classification**: Evaluate if a reported issue is a valid defect/request or a misunderstanding. Not all requests should be handled. If adding a requested feature or fixing a non-critical bug risks affecting multiple components or destabilizing the library, skip it. Priority is **stability over feature perfection**. If it can be handled by explaining the behavior or pointing to an existing example in the `example/` app, do that instead of changing the code. In such cases, answer the ticket clearly and close it.
