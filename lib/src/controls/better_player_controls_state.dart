...
abstract class BetterPlayerControlsState<T extends StatefulWidget> extends State<T> {
  ...
  void skipBack() {
    if (latestValue != null) {
      ...
      final skip = (latestValue!.position - Duration(milliseconds: betterPlayerControlsConfiguration.backwardSkipTimeInMilliseconds)).inMilliseconds;
  ...
  void skipForward() {
    if (latestValue != null) {
      ...
      final skip = (latestValue!.position + Duration(milliseconds: betterPlayerControlsConfiguration.forwardSkipTimeInMilliseconds)).inMilliseconds;
  ...
  Widget _buildSubtitlesSourceRow(BetterPlayerSubtitlesSource subtitlesSource) {
    ...
    final bool isSelected = (subtitlesSource == selectedSourceType) || (subtitlesSource.type == BetterPlayerSubtitlesSourceType.none && subtitlesSource.type == selectedSourceType!.type);
  ...
  TextStyle _getOverflowMenuElementTextStyle(bool isSelected) {
    return TextStyle(
      ...
      color: isSelected ? betterPlayerControlsConfiguration.overflowModalTextColor : betterPlayerControlsConfiguration.overflowModalTextColor.withOpacity(0.7),
    );
  }
  ...
  void _showMaterialBottomSheet(List<Widget> children) {
    // very bad workaround
    Future.delayed(Duration(milliseconds: 50), () => betterPlayerController?.postEvent(BetterPlayerEvent(BetterPlayerEventType.overflowOpened)));
