...
abstract class BetterPlayerControlsState<T extends StatefulWidget> extends State<T> {
  ...
  void skipBack() {
    ...
    final skip = (latestValue.position - Duration(milliseconds: betterPlayerControlsConfiguration.skipsTimeInMilliseconds)).inMilliseconds;
        ...
        child: Row(
          children: [
            ...
            Text(
              "$value x",
              style: TextStyle(
                  fontWeight: betterPlayerController.videoPlayerController.value.speed == value ? FontWeight.bold : FontWeight.normal),
            )
          ],
        ),
      ),
  ...
  void _showSubtitlesSelectionWidget() {
    ...
    final noneSubtitlesElementExists =
        subtitles?.firstWhere((source) => source.type == BetterPlayerSubtitlesSourceType.none, orElse: () => null) != null;
    ...
    showModalBottomSheet<void>(
      ...
      builder: (context) {
        return SafeArea(
          ...
          child: SingleChildScrollView(
            child: Column(
              children: subtitles.map((source) => _buildSubtitlesSourceRow(source)).toList(),
            ),
          ),
        );
      },
  ...
  void _showQualitiesSelectionWidget() {
    final List<String> trackNames = betterPlayerController.betterPlayerDataSource.hlsTrackNames ?? [];
    final List<BetterPlayerHlsTrack> tracks = betterPlayerController.betterPlayerTracks;
    final List<Widget> children = [];
    for (var index = 0; index < tracks.length; index++) {
      final preferredName = trackNames.length > index ? trackNames[index] : null;
    ...
    if (children.isEmpty) {
      children.add(_buildTrackRow(BetterPlayerHlsTrack(0, 0, 0), betterPlayerController.translations.generalDefault));
    }
  ...
  Widget _buildTrackRow(BetterPlayerHlsTrack track, String preferredName) {
    ...
    final String trackName = preferredName ?? "${track.width}x${track.height} ${BetterPlayerUtils.formatBitrate(track.bitrate)}";
