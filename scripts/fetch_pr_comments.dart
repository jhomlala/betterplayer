...
void main(List<String> args) async {
  if (args.isEmpty) {
    ...
    print('Example (defaults to jhomlala/betterplayer): dart run fetch_pr_comments.dart 1499');
    print('Example (custom repo): dart run fetch_pr_comments.dart 123 flutter/flutter');
    exit(1);
  }
...
''';
  ...
  try {
    while (hasNextPage) {
      ...
      if (pr == null) {
        print('Error: Could not find PR #$prNumber in repository $owner/$repo.');
        exit(1);
      }
      ...
      if (variables['cursor'] == null) {
            ...
            if (thread['comments']['nodes'] != null) {
              for (var comment in thread['comments']['nodes']) {
                allComments.add({
                  ...
                  "author": comment['author'] != null ? comment['author']['login'] : "Unknown",
        ...
        if (pr['reviews']['nodes'] != null) {
          for (var review in pr['reviews']['nodes']) {
            if (review['body'] != null && review['body'].toString().isNotEmpty) {
              allComments.add({
                "type": "review_summary",
                "author": review['author'] != null ? review['author']['login'] : "Unknown",
    ...
    allComments.sort((a, b) => (a['createdAt'] as String).compareTo(b['createdAt'] as String));
    ...
    await outputFile.writeAsString(const JsonEncoder.withIndent('  ').convert(allComments));
